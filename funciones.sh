...
Login()
{
    ...
    while IFS=":" read -r usuDb pwdDb; 
    do 
    
        if [[ "$usu" == "$usuDb" && "$pwd"=="$pwdDb" ]];
        then                       
            return 0
        fi
...
ControladorOpcionesMenu() {
    ...
    case $opcion in
        ...
        2)
            ...
            ;;
        3)
            echo "Has seleccionado la Opción Configurar letra inicial"
            
            ;;
        4)
            echo "Has seleccionado la Opción Configurar letra final"
            
            ;;
        5)
            echo "Has seleccionado la Opción Configurar letra contenida"
